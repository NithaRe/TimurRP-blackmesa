# SPDX-FileCopyrightText: 2025 Aiden <28298836+Aidenkrz@users.noreply.github.com>
# SPDX-FileCopyrightText: 2025 Aiden <aiden@djkraz.com>
#
# SPDX-License-Identifier: AGPL-3.0-or-later

rmc-ui-link-discord-account = Привязать аккаунт Discord
rmc-ui-link-discord-account-text =
    Нажмите кнопку ниже, чтобы скопировать ваш код.
    Затем перейдите в канал #account-linking для привязки аккаунтов.
rmc-ui-link-discord-account-copy = Копировать код
rmc-ui-link-discord-account-copied = Скопировано!
rmc-ui-link-discord-account-already-linked = [bold]Ваш аккаунт уже привязан![/bold]
rmc-ui-link-discord-account-open-channel = Открыть канал
rmc-ui-patron-perks = Привилегии Патрона
rmc-ui-lobby-message = Сообщение в лобби
rmc-ui-lobby-message-description =
    Отображается игрокам перед началом раунда.
    Одно сообщение выбирается случайным образом из всех Патронов этого уровня.
    Убедитесь, что выбранное сообщение не нарушает правила.
    Если не уверены, спросите в AHelp! (F1)
rmc-ui-shoutout = Объявление в конце раунда
rmc-ui-shoutout-nt-title = Имя для объявления Black Mesa в конце раунда.
rmc-ui-shoutout-nt-description = Отображается в конце раунда как "Black Mesa финансируется: " и ваше имя.
rmc-ui-shoutout-example = Пример:
rmc-ui-shoutout-info =
    Каждое имя выбирается случайным образом из всех Патронов этого уровня.
    Убедитесь, что выбранные имена в рамках ролевой игры и не нарушают правила.
    Если не уверены, спросите в AHelp! (F1)
rmc-ui-save = Сохранить
rmc-ui-ghost-color = Цвет призрака
rmc-ui-ghost-color-clear = Очистить
rmc-ui-ghost-color-save = Сохранить
rmc-ui-shoutout-nt = [bold]Black Mesa финансируется: [color=green]{ $name }[/color]![/bold]
rmc-ui-discord = Discord
rmc-ui-patreon = Patreon
