...
ent-EmergencyLight = аварийная лампа
    .desc = Маленькая лампочка с внутренним аккумулятором, которая включается, как только перестаёт получать питание извне. Технология Black Mesa позволяет ей адаптировать свой цвет для оповещения персонала об обстановке в комплексе.
