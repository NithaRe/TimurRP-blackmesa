ent-ClothingNeckBronzeheart = медаль бронзовое сердце
    .desc = Выдаётся членам персонала за проявленную храбрость перед лицом опасности.
ent-ClothingNeckGoldmedal = золотая медаль мастерства персонала
    .desc = Выдаётся членам персонала, проявившим отличные навыки работы в команде.
ent-ClothingNeckCargomedal = медаль сотрудника снабжения
    .desc = Вручается лучшему работнику отдела снабжения.
ent-ClothingNeckEngineermedal = медаль инженерного работника
    .desc = Вручается лучшему работнику инженерного отдела.
ent-ClothingNeckMedicalmedal = медаль медицинского работника
    .desc = Вручается лучшему работнику медицинского отдела.
ent-ClothingNeckSciencemedal = медаль научного сотрудника
    .desc = Вручается лучшему работнику научного отдела.
ent-ClothingNeckSecuritymedal = медаль службы безопасности
    .desc = Вручается лучшему работнику отдела службы безопасности.
ent-ClothingNeckClownmedal = медаль клоуна
    .desc = Вручается за лучшую шутку во Вселенной. ХОНК!
