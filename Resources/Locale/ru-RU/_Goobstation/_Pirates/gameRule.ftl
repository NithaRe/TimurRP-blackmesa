# SPDX-FileCopyrightText: 2025 Aiden <28298836+Aidenkrz@users.noreply.github.com>
# SPDX-FileCopyrightText: 2025 Aiden <aiden@djkraz.com>
# SPDX-FileCopyrightText: 2025 amogus <113782077+whateverusername0@users.noreply.github.com>
#
# SPDX-License-Identifier: AGPL-3.0-or-later

antag-pirate-briefing =
    комплекс отказалась платить за вашу защиту!
    
    Вам нужно совершить налёт на станцию и забрать всё ценное. Как только вы соберёте достаточно, продайте добычу через грузовой поддон и вставьте кредиты в сифон данных на вашем корабле.
    Захватите как можно больше добычи!
    
    При активации сифона данных вы начнёте высасывать кредиты из отдела снабжения комплекса.
    Будьте осторожны — это заякорит ваш корабль, оповестит персонал и приведёт его в ярость, так что готовьтесь к серьёзной битве!
antag-pirate-briefing-short =
    Вы — пират.
    Защищайте корабль, выкачивайте кредиты с комплекса и грабьте её ради ещё большей добычи!
pirate-roundend-append = Пиратская команда ограбила станцию на сумму в [color=yellow]{ $num }[/color] кредитов!
pirate-roundend-append-siphon = [color=green]Пиратам удалось выкачать[/color] [color=yellow]{ $num }[/color] [color=green]кредитов с комплекса![/color]
pirate-roundend-append-lose = [color=red]Пираты потеряли свой банк данных![/color]
pirate-roundend-list = Пиратами были:
